...
def run_sudo_command(client, command, timeout=180):
    """Execute a sudo command with password"""
    print(f"\n>>> sudo {command}")
    # Use echo to pipe password to sudo -S
    full_command = f"echo '{SSH_PASS}' | sudo -S bash -c '{command}'"
    return run_command(client, full_command, timeout)
...
def main():
    ...
    try:
        ...
        run_sudo_command(client, "bash /tmp/nodesource_setup.sh", timeout=120)

        # Install Node.js 20
        run_sudo_command(client, "apt-get install -y nodejs", timeout=300)
        ...
        run_sudo_command(client, "pkill -f 'next' 2>/dev/null || true")
        time.sleep(2)
